...
def fetch_ohlcv_binance_rest(symbol: str, timeframe: str, limit: int = 500) -> pd.DataFrame:
    """Direct Binance REST API — pure Python, no ccxt needed."""
    try:
        import requests
...
def fetch_data(symbol: str, timeframe: str) -> pd.DataFrame:
    """Fetch with automatic fallback: Binance REST → CCXT → yFinance → Demo."""
    df = fetch_ohlcv_binance_rest(symbol, timeframe)
    if df.empty and CCXT_AVAILABLE:
